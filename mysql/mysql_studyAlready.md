...
--------------------
mysql> select ename sal from emp where sal>min(800);
...
重点结论：在一条select语句当中，如果有group by语句的话，select后面只能跟：**参加分组的字段，以及分组函数。**其它的一律不能跟。（其他跟了没有意义）
...
ERROR 1111 (HY000): Invalid use of group function
mysql>
min(sal)算是分组函数，必须要先分组才能够使用，where使用的时候还没有分组，所以不能使用分组函数
mysql> select ename,sum(sal) from emp;
+-------+----------+
| ename | sum(sal) |
+-------+----------+
| SMITH | 29025.00 |
+-------+----------+
1 row in set (0.00 sec)
这个可以使用的原因，是因为在使用sum分组函数之前，已经把整张表当作一组了
----
先分组再筛选
mysql> select deptno,max(sal) from emp group by deptno having max(sal)>3000;
+--------+----------+
...
1 row in set (0.01 sec)
```
显示岗位平均工资，要求平均工资大于1500，除manager岗位之外，按照降序排列。
...
两张表连接的时候没有任何限制的时候，最终查询条数是两张表的乘积，这种现象被称作笛卡尔积现象。

避免方法：连接时加条件，让满足条件的记录才被筛选出来
...
通过笛卡儿积现象得出，表的连接次数越多效率越低，尽量避免表的链接
```
...
mysql> select e.ename,d.dname from emp e,dept d where e.deptno=d.deptno;#mysql92语法
表的结果不清晰，表的连接条件和后期的筛选都放到了where
+--------+------------+
| ename  | dname      |
+--------+------------+
| SMITH  | RESEARCH   |
...
mysql> select e.ename,d.dname from emp e join dept d on e.deptno=d.deptno;#mysql99语法
sql99优点：表结构独立，链接之后如果还需要进一步筛选，继续往后加where
+--------+------------+
| ename  | dname      |
+--------+------------+
| SMITH  | RESEARCH   |
...
查询员工的上级名
mysql> select a.ename as '员工名',b.ename as '上级名' from emp a left join emp b on a.mgr=b.empno;
+--------+--------+
| 员工名 | 上级名 |
+--------+--------+
| SMITH  | FORD   |
...
找到每个员工部门名称，工资等级，还有上级领导，显示员工名，领导名，部门名字，薪资和薪资等级。
mysql>  select e.ename as '员工部门名称',e.sal as '薪资', s.grade as '薪资等级',d.dname as '部门名字',e2.mgr as '上级领导'  from emp e  join dept d   on e.deptno=d.deptno  left join emp e2  on e.mgr = e2.empno join salgrade s  on e.sal between s.losal and s.hisal;
+--------------+---------+----------+------------+----------+
| 员工部门名称 | 薪资    | 薪资等级 | 部门名字   | 上级领导 |
+--------------+---------+----------+------------+----------+
| SMITH        |  800.00 |        1 | RESEARCH   |     7566 |
| ALLEN        | 1600.00 |        3 | SALES      |     7839 |
| WARD         | 1250.00 |        2 | SALES      |     7839 |
| JONES        | 2975.00 |        4 | RESEARCH   |     NULL |
| MARTIN       | 1250.00 |        2 | SALES      |     7839 |
| BLAKE        | 2850.00 |        4 | SALES      |     NULL |
| CLARK        | 2450.00 |        4 | ACCOUNTING |     NULL |
| SCOTT        | 3000.00 |        4 | RESEARCH   |     7839 |
| KING         | 5000.00 |        5 | ACCOUNTING |     NULL |
| TURNER       | 1500.00 |        3 | SALES      |     7839 |
| ADAMS        | 1100.00 |        1 | RESEARCH   |     7566 |
| JAMES        |  950.00 |        1 | SALES      |     7839 |
| FORD         | 3000.00 |        4 | RESEARCH   |     7839 |
| MILLER       | 1300.00 |        2 | ACCOUNTING |     7839 |
+--------------+---------+----------+------------+----------+
